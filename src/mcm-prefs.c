...
static void mcm_prefs_profile_store_changed_cb (McmProfileStore *_profile_store, gpointer user_data);
...
mcm_prefs_startup_phase1_idle_cb (gpointer user_data)
{
	...
	mcm_prefs_set_combo_simple_text (widget);
	intent_display = g_settings_get_enum (settings, MCM_SETTINGS_RENDERING_INTENT_DISPLAY);
	...
	mcm_prefs_set_combo_simple_text (widget);
	intent_softproof = g_settings_get_enum (settings, MCM_SETTINGS_RENDERING_INTENT_SOFTPROOF);
 ...
 **/
static void
mcm_prefs_graph_combo_changed_cb (GtkWidget *widget, gpointer data)
{
	gint active;
	...
	g_settings_set_enum (settings, MCM_SETTINGS_PROFILE_GRAPH_TYPE, active);
}
 ...
 **/
static void
mcm_prefs_setup_graph_combobox (GtkWidget *widget)
{
	gint active;
	...
	/* get from settings */
	active = g_settings_get_enum (settings, MCM_SETTINGS_PROFILE_GRAPH_TYPE);
...
main (int argc, char **argv)
{
	...
	mcm_prefs_set_combo_simple_text (widget);
	mcm_prefs_setup_graph_combobox (widget);
	g_signal_connect (widget, "changed",
			  G_CALLBACK (mcm_prefs_graph_combo_changed_cb), (gpointer) "cmyk");
...
//	gtk_scale_add_mark (GTK_SCALE (widget), 1.0f, GTK_POS_TOP, "");
	...
	/* use vcgt widget */
	vcgt_widget = mcm_trc_widget_new ();
	...
	/* refresh UI */
	widget = GTK_WIDGET (gtk_builder_get_object (builder, "combobox_graph"));
	mcm_prefs_graph_combo_changed_cb (widget, NULL);
